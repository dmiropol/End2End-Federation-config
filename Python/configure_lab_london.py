...
nsx_ip = "192.168.113.201"
#kvm_ip = "192.168.130.151"
vc_ip = "192.168.113.22"
...
# Load json file
def load_json(file):
    with open('./request_body_london/' + file) as json_file:
...
def define_TEP_IP_pool():
    ...
    # No easy way to check for realization. Without the sleep creation
    # could fail
    time.sleep(120)
    return True
...
def poweron_kvm_vms():
    ...
    for vm in kvm_vms:
        ...
        [stdin, stdout, stderr] = execute_command(kvm_ip, kvm_ssh_port, "root", "VMware1!", command)
...
def create_tier0():
    ...
    # Assumes only 1
    edge_node_member_index = {}
    r = get('/api/v1/edge-clusters', ignore_plan=True)
    ec = r.json()['results'][0]
    for e_n in ec['members']:
        edge_node_member_index[e_n['display_name']] = str(e_n['member_index'])
    ...
    for iface in request_body['interfaces']:
        if (iface['edge_path'] == "EDGE1_PATH"):
            edge_path = '/infra/sites/default/enforcement-points/default/edge-clusters/' + edge_cluster_id + \
                        '/edge-nodes/' + edge_node_member_index['edgenode-01a']
        elif (iface['edge_path'] == "EDGE2_PATH"):
            edge_path = '/infra/sites/default/enforcement-points/default/edge-clusters/' + edge_cluster_id + \
                        '/edge-nodes/' + edge_node_member_index['edgenode-02a']
